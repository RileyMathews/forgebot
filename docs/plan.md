...
- [x] Implement startup crash recovery in `main.rs`: query `get_sessions_in_state(&["planning","building"])`, set each to `error`, post a comment on the issue explaining forgebot restarted mid-run
- [x] Confirm: invoke `run_opencode` against a real repo worktree with a simple prompt and verify opencode runs and exits
